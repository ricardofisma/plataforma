...
class Spritz {
  ...
  constructor(selector, options = {}) {
    ...
    if (!Array.isArray(this.options.picture))
      this.options.picture = [this.options.picture];
    ...
    return this.options.init > 0 ? this.init(this.options.init) : this;
  }
  ...
  _animate(timestamp) {
    ...
    if (this.initiated) {
      ...
      let seg = Math.floor(
        (timestamp - this.animTime) / (1000 / this.currentFps)
      );
      let pauseAnim = false;
  ...
  _setDimensions() {
    ...
    if (this.pic.objectFit === "cover") {
      if (this.stepRatio >= this.parentRatio) {
        ...
        this.canvasWidth =
          (this.stepWidth * this.canvasHeight) / this.stepHeight;
      } else {
        this.canvasWidth = this.parentWidth;
        this.canvasHeight =
          (this.stepHeight * this.canvasWidth) / this.stepWidth;
      }
    } else {
      if (this.stepRatio >= this.parentRatio) {
        this.canvasWidth = this.parentWidth;
  ...
  _createCanvas() {
    ...
    this.canvas.setAttribute(
      "style",
      "position:absolute;left:50%;top:50%;-webkit-transform:translateY(-50%) translateY(1px) translateX(-50%) translateX(1px);-ms-transform:translateY(-50%) translateY(1px) translateX(-50%) translateX(1px);transform:translateY(-50%) translateY(1px) translateX(-50%) translateX(1px);"
    );
